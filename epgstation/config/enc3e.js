...
Array.prototype.push.apply(args, ['-ss', '5', '-y']);
...
// ビデオストリーム設定
Array.prototype.push.apply(args, ['-map', '0:v:0', '-metadata:s:v:0', 'title=' + chname, '-c:v', 'libx265']);

// インターレス解除
Array.prototype.push.apply(args, ['-vf', 'yadif,scale=-2:720']);

// 複数音声放送対応（二か国語、音声多重、音声解説）
Array.prototype.push.apply(args, ['-metadata:s:a:0', 'title=主音声', '-metadata:s:a:1', 'title=副音声']);

if (name.indexOf('[二]') != -1 || name.indexOf('[多]') != -1 || name.indexOf('[解]') != -1) {
    if (isDualMono) {
...
// 字幕ストリーム設定
if (name.indexOf('[字]') != -1 || name.indexOf('[字/') != -1 || chname.indexOf('ディズニーch') != -1) {
    Array.prototype.push.apply(args, [
        '-map', '0:s?',
        '-c:s', 'mov_text',
        '-metadata:s:s:0', 'title=ARIB字幕',
